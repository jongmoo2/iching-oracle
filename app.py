...
def call_gemini_ai(prompt_text):
    ...
    가급적 물상을 기준으로 상괘(1~8), 하괘(1~8), 동효(1~6)를 도출하되 상황에 따라 인물, 사건, 감정 등도 고려하여 도출할 수 있습니다.
    동효를 도출할 때는 물상이나 상황에 따라 숫자(1~6)를 선택하되, 점치는 시간을 더해서 6으로 나눈 나머지(1~6)를 최종 동효로 선택해주세요.
