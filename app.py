...
def call_gemini_ai(prompt_text):
    ...
    # 주역점 전문 프롬프트 (상수학 및 매화역수 알고리즘 강화)
    full_prompt = f"""
    [역할 정의]
    ...
    [사용자 입력 데이터]
    - 질문/상황: "{prompt_text}"
    - 기준 시각 (한국 표준시 24시간제 정수): 현재 {json.dumps(st.datetime.datetime.now().hour if 'datetime' in globals() else 19)}시 부근 (이를 참고하여 아래 수리 연산을 정확히 수행하라.)
    ...
    2. 동효(動爻) 계산 규칙 (★절대 변조 불가 수학 공식):
       - 1단계: 상황에 나오는 특정 숫자, 문장의 글자 수, 자음/모음 수, 혹은 소성괘 수의 합 등을 바탕으로 AI 네가 직관적으로 판단한 '기본 동효 숫자(1~6)'를 먼저 하나 정한다.
       - 2단계: 만약 상황에서 동효 숫자를 정할 수 없는 경우에 현재 점치는 시간(위에 주어진 24시간제 기준 시(Hour) 정수)을 6으로 나눈 나머지로 동효를 정한다.
         * 만약 나머지가 0이 나오면, 최종 동효는 반드시 '6'으로 결정한다. (나머지가 1~5면 그대로 1~5효)
         * 산수 오류는 절대 용납되지 않는다. 검산 후 최종 값을 도출하라.
    
    3. 만약 상괘와 하괘 동효를 물상이나 상황으로 정할 수 없으면 현재 점치는 시간(위에 주어진 24시간제 기준으로 하라.)에서 시(Hour)를 8로 나눈 나머지로 상괘를 정하고, 분(minute)을 8로 나눈 나머지로 하괘를 정하고, 초(secomds)를 6으로 나눈 나머지로 동효를 정한다.
    ...
    {{
      ...
      "lower": 숫자(1~8),
      "moving": 숫자(1~6),
      "rationale": "상·하괘를 선정한 구체적 물상학적 근거(물상이 안 될 때 상황을 적용한 이유 포함) 및 동효 연산 과정 [예시: 기본동효(X) + 현재시간(Y) = Z, Z % 6 = 최종동효]을 명확히 기록한 분석 (한국어)",
