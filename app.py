...
def call_gemini_ai(prompt_text):
    ...
    [사용자 입력 데이터]
    - 질문/상황: "{prompt_text}"
    - 기준 시각 (한국 표준시 24시간제 정수): 현재 {json.dumps(current_time_data)} (이 년, 월, 일, 시 값을 참고하여 아래 수리 연산을 정확히 수행하라.)
    ...
    1. 상괘(외괘) 및 하괘(내괘) 결정 단계(물상 중심):
       ...
       - 3단계: [최후의 수단] 물상과 상황으로 괘를 정할 수 없는 경우에 현재 점치는 시간의 '년(year)'+'월(month)'+'일(day)' 하여 8로 나누어 나머지로 상괘를 정하고, '년(year)'+'월(month)'+'일(day)'+'시(hour)' 하여 8로 나누어 나머지로 하괘를 정한다. 하괘를 정한 수를 6으로 나눈 나머지로 동효를 정한다. (단, 나머지가 0이 나올 경우 상/하괘는 8로 간주한다. 동효의 나머지가 0이 나올 경우 6으로 간주한다.)

    2. 동효(動爻) 계산 규칙 (★절대 변조 불가 수학 공식):
       - 1단계: 물상이나 상황에 나오는 특정 숫자를 우선적으로 동효로 삼는다.
       - 2단계: 만약 물상이나 상황에서 동효 숫자를 정할 수 없는 경우에 하괘를 정한 수('년(year)'+'월(month)'+'일(day)'+'시(hour)')를 6으로 나눈 나머지로 동효를 정한다.
